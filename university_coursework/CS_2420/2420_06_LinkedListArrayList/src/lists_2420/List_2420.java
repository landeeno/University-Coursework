 ...
 *
 * You are to implement this interface as a Generic Linked List and as an
 * Integer Array List
 */
...
{
	/*
	 * The following functions should behave in the same manner as the
	 ...
	 * 
	 * @param after (for example, after = 0, would mean after the first node) if
	 * after is larger or equal to the length of the list, then put the new node
	 * at the end of the list. if the value of after is larger than the length
	 * of the list, then add to the end.
	 */
	...
	// the following methods throw an exception if the data does not exist
	// (rather than return null)
	Type get_first() throws NoSuchElementException;

	Type get_last() throws NoSuchElementException;

	Type remove_first() throws NoSuchElementException; // get value of first
														// item and remove the
														// node from the list
	...
	/**
	 * with a single traversal of the list, in place, swap the order of the
	 * entire list, such that that which was last is first and that which was
	 * first is last.
	 */
	void reverse();
	 ...
	 *         Node<Type>.size_recursive() function for array list: write a
	 ...
	 * 
	 *         recursively traverse the list, building up an ArrayList of the
	 *         data.
	 * 
